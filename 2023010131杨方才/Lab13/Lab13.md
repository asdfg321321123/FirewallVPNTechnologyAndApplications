...
| :--- | :--- | :--- |
| `gwA` |10.2.0.0/24 |10.2.0.0/24 |
| `gwB` | 10.1.0.0/24|10.1.0.0/24 |
...
| 网关 | 允许规则 | 日志前缀 | 默认策略 |
| :--- | :--- | :--- | :--- |
| `gwA` | 允许 10.1.0.0/24 → 10.2.0.2:8080|A-TO-B-DENY: |DROP |
| `gwB` | 允许 10.1.0.0/24 → 10.2.0.2:8080|VPN-S2S-DENY:	 | DROP|
...
| `siteA -> siteB:2222` | 失败 |失败 |有 A-TO-B-DENY 日志 |
| `siteA -> siteB ping` | 失败 | 失败| 有 A-TO-B-DENY 日志|
...
| `latest handshake` | 10 seconds ago|55 seconds ago |
| `transfer` 计数 |456 B sent|526 B sent |
...
| 测试 | 结果 | 命中的规则 |
| :--- | :--- | :--- |
| `siteA -> siteB:8080` |成功 | gwA/gwB 的 8080 允许规则|
| `siteA -> siteB:2222` |失败 |gwA 的 LOG + REJECT 规则 |
| `siteA -> siteB ping` |失败 | gwA 的 LOG + REJECT 规则|

### E. 抓包对比

| 抓包位置 | 看到的地址 | 协议 | 说明 |
| :--- | :--- | :--- | :--- |
| underlay | 192.0.2.1 ↔ 192.0.2.2	|UDP 51820| 外层加密包（WireGuard 封装）|
| `wg0` | 10.1.0.2 ↔ 10.2.0.2| TCP 8080|解密后的跨站点业务包 |
| B 站点 LAN | 10.1.0.2 ↔ 10.2.0.2| TCP 8080|转发到 B 站点内网的业务包 |
